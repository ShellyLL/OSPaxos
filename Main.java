...
public class Main {
      ...
      while(true)
      {
         try{
            ...
            if(cmd.equalsIgnoreCase("help"))
               ...
               failAndRecoverDemo(); // random pick a failed node and after some time, recover it   
            else
   ...
   private static void leaderRecover() {
      ...
      writeDebug("Leader " + node1.getLocationData().getNodeID() + " is Recovered");
      node1.becomeLeader();
      addToSetAndMap(node1);
      while (!failedLeaders.isEmpty()) {
         Node node2 = failedLeaders.poll();
         writeDebug("Node " + node2.getLocationData().getNodeID() + " is Recovered");
   ...
   private static void voterRecover() {
      ...
      while (!failedVoters.isEmpty()) {
         ...
         writeDebug("Node " + node.getLocationData().getNodeID() + " is Recovered");
         addToSetAndMap(node);
      }
      resetSetAndMap();
   }
