...
public class Main2 {
   ...
   private static void read(Set<Node2> nodes) {
   ...
   private static void write(String v, Set<Node2> nodes) {
   ...
   private static void addToSetAndMap(Node2 node) {
      nodes.add(node);
      nodeLocationSet.add(node.getLocationData());
      nodeLocationMap.put(node.getLocationData(), node);
   }

   private static void removeFromSetAndMap(Node2 node) {
      nodes.remove(node);
